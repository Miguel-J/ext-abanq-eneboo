...
function prod_datosPedidoRS(eLinea, curRS)
{
  var _i = this.iface;
  var fecha = eLinea.attribute("FechaPedido");

  var codEjercicio = curRS.valueBuffer("codejercicio");
  ...
  var codProveedor = eLinea.attribute("CodProveedor");
  with(_i.curPedidoRS) {
    setValueBuffer("codproveedor", codProveedor);
    setValueBuffer("nombre", AQUtil.sqlSelect("proveedores", "nombre", "codproveedor = '" + codProveedor + "'"));
    setValueBuffer("cifnif", AQUtil.sqlSelect("proveedores", "cifnif", "codproveedor = '" + codProveedor + "'"));
