...
function prod_bufferChanged(fN: String)
{
  ...
  switch (fN) {
    ...
    case "refcomponente": {
      ...
      //      } else {
      if (!this.iface.desdeTareas) {
        if (this.child("fdbIdTipoTareaPro")) {
          this.child("fdbIdTipoTareaPro").setDisabled(false);
        }
        if (this.child("fdbDiasAntelacion")) {
    ...
    case "refcompuesto": {
      sys.setObjText(this, "fdbIdTipoOpcionArt", "");
      sys.setObjText(this, "fdbIdOpcionArticulo", "");
      if (this.child("fdbIdTipoOpcionArt")) {
