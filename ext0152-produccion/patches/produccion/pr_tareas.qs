...
function prod_commonCalculateField(fN, cursor)
{
  ...
  var util = new FLUtil;
  switch (fN) {
    case "tiempotrabajadores": {
      var idTarea = cursor.valueBuffer("idtarea");
      valor = parseFloat(util.sqlSelect("pr_tareastrabajador", "SUM(totalacumulado)", "idtarea = '" + idTarea + "'"));
...
function prod_tbnStopPrep_clicked()
{
  ...
  switch (subestado) {
    case util.translate("scripts", "PREPARACIÓN EN CURSO"): {
      ...
      cursor.setValueBuffer("tiempotrabajadores", this.iface.commonCalculateField("tiempotrabajadores", cursor));
