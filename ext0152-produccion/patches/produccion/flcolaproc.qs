...
class prod extends oficial
{
  ...
  {
    return this.ctx.prod_controlBorradoProcesoProd(curProceso);
  }
  ...
  //  function calcularTiempoTrabajadores(idTarea) {
  //    return this.ctx.prod_calcularTiempoTrabajadores(idTarea);
  //  }
  ...
  function totalizarTiempoTrabajadoresTarea(curTT)
  {
    return this.ctx.prod_totalizarTiempoTrabajadoresTarea(curTT);
  }
  function verificarTiempoTrabajadoresTarea(curTT)
  {
    return this.ctx.prod_verificarTiempoTrabajadoresTarea(curTT);
  }
  ...
  function afterCommit_pr_tareastrabajador(curTT)
  {
    return this.ctx.prod_afterCommit_pr_tareastrabajador(curTT);
  }
  //  function iniciarTareaEsp(curTareas:FLSqlCursor):Boolean {
  //    return this.ctx.prod_iniciarTareaEsp(curTareas);
  //  }
...
function prod_iniciarTareaEsp(curTareas: FLSqlCursor): Boolean {
  ...
  curMoviStock.setForwardOnly(true);
  curMoviStock.select("codloteprod = '" + codLote + "' AND idtipotareapro = " + curTareas.valueBuffer("idtipotareapro") + " AND idproceso = " + curTareas.valueBuffer("idproceso"));
...
function prod_terminarTareasActivas(idTarea, estado)
{
  var util: FLUtil;
  var curTareasTrab = new FLSqlCursor("pr_tareastrabajador");
  curTareasTrab.setActivatedCommitActions(false); /// Evita que se llame al cálculo del total horas trabajador (la llamada se hace posteriormente)
...
\end */
function prod_beforeCommit_pr_procesos(curProceso)
{
  var _i = this.iface;
  if (!_i.controlBorradoProcesoProd(curProceso)) {
    return false;
  }
  return true;
}

function prod_controlBorradoProcesoProd(curProceso)
{
  var util: FLUtil = new FLUtil;
...
function prod_parametrosProceso(curProceso: FLSqlCursor): String {
  ...
  if (!this.iface.xmlDocParametros_)
  {
    debug("prod_parametrosProceso 1");
    return false;
  }
...
function prod_crearProcesoProd(referencia: String, codLote: String, idLineaPedidoCli: String): Number {
  ...
  this.iface.curProceso_.setValueBuffer("referencia", referencia);
  ...
  if (datosProceso_)
  {
    ...
    if (datosProceso_["tipoproduccion"] == "M") {
      debug("llega");
      if (!flfactalma.iface.pub_crearComposicion(curLote, false, referencia, idProceso, true))
        return false;
    }
  }
  if (!this.iface.asignarTareas(idProceso, idTipoProceso))
  {
    return false;
  }
...
// function prod_calcularTiempoTrabajadores(idTarea)
// {
//  var util:FLUtil;
//  var tiempo:Number = parseFloat(util.sqlSelect("pr_tareastrabajador","SUM(totalacumulado)","idtarea = '" + idTarea + "'"));
//  return tiempo;
// }
...
function prod_calcularTiemposFinalizacionTarea(curTareas, fechaFin)
{
  if (!this.iface.__calcularTiemposFinalizacionTarea(curTareas, fechaFin))
    return false;

  curTareas.setValueBuffer("tiempotrabajadores", formRecordpr_tareas.iface.pub_commonCalculateField("tiempotrabajadores", curTareas));
...
function prod_afterCommit_pr_tareastrabajador(curTT)
{
  if (!this.iface.verificarTiempoTrabajadoresTarea(curTT)) {
    return false;
  }
  return true;
}

function prod_totalizarTiempoTrabajadoresTarea(idTarea)
{
  ...
  curTarea.refreshBuffer();
  curTarea.setValueBuffer("tiempotrabajadores", formRecordpr_tareas.iface.pub_commonCalculateField("tiempotrabajadores", curTarea));
  if (!curTarea.commitBuffer()) {
    return false;
  }
  return true;
...
function prod_verificarTiempoTrabajadoresTarea(curTT)
{
  switch (curTT.modeAccess()) {
    case curTT.Edit: {
      var idTarea = curTT.valueBuffer("idtarea");
      var idTareaP = curTT.valueBufferCopy("idtarea");
      if (idTarea != idTareaP) {
        if (!this.iface.totalizarTiempoTrabajadoresTarea(idTarea)) {
          return false;
        }
        if (!this.iface.totalizarTiempoTrabajadoresTarea(idTareaP)) {
          return false;
        }
      } else {
        if (curTT.valueBuffer("totalacumulado") != curTT.valueBufferCopy("totalacumulado")) {
          if (!this.iface.totalizarTiempoTrabajadoresTarea(curTT.valueBuffer("idtarea"))) {
            return false;
          }
        }
      }
    ...
    case curTT.Insert:
    case curTT.Del: {
      var tA = curTT.valueBuffer("totalacumulado");
      if (tA && !isNaN(tA)) {
        if (!this.iface.totalizarTiempoTrabajadoresTarea(curTT.valueBuffer("idtarea"))) {
          return false;
        }
