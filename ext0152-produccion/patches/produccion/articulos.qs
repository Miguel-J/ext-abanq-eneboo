...
class prod extends articuloscomp
{
  ...
  function validarProduccion(): Boolean {
    return this.ctx.prod_validarProduccion();
  }
  function validarTareaProduccion(): Boolean {
    return this.ctx.prod_validarTareaProduccion();
  }
}
//// PRODUCCIÓN /////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
...
function prod_init()
{
  ...
  if (sys.isLoadedModule("flcolaproc")) {
    if (cursor.valueBuffer("fabricado")) {
      sys.enableObj(this, "fdbDiasProdEnvio");
      sys.enableObj(this, "fdbIdTipoProceso");
    }
  } else {
...
function prod_validarProduccion(): Boolean {
  ...
  if (cursor.valueBuffer("fabricado"))
  {
    ...
    if (!idTipoProceso || idTipoProceso == "") {
      MessageBox.warning(AQUtil.translate("scripts", "Si el artículo es fabricado, debe indicar cuál es su proceso de fabricación"), MessageBox.Ok, MessageBox.NoButton);
      return false;
    }

    if (!AQUtil.sqlSelect("pr_tiposproceso", "fabricacion", "idtipoproceso = '" + idTipoProceso + "'")) {
      MessageBox.warning(AQUtil.translate("scripts", "El tipo de proceso %1 no está marcado como Proceso de fabricación.\nModifique este tipo de proceso o elija uno distinto que sí esté marcado.").arg(idTipoProceso), MessageBox.Ok, MessageBox.NoButton);
      return false;
    }

    var tipoProduccion: String = AQUtil.sqlSelect("pr_tiposproceso", "tipoproduccion", "idtipoproceso = '" + idTipoProceso + "'");
    switch (tipoProduccion) {
      case "Fabricación": {
        if (cursor.valueBuffer("tipostock") != "Lotes") {
          MessageBox.warning(AQUtil.translate("scripts", "El proceso asociado al artículo es de fabricación. El tipo de stock debe ser Lotes"), MessageBox.Ok, MessageBox.NoButton);
          return false;
        }
        if (AQUtil.sqlSelect("articulosaplicables", "id", "refproceso = '" + cursor.valueBuffer("referencia") + "'")) {
          MessageBox.warning(AQUtil.translate("scripts", "El proceso asociado al artículo es de fabricación. No debe tener artículos aplicables"), MessageBox.Ok, MessageBox.NoButton);
          return false;
        }
        break;
      }
      case "Modificación": {
        if (!cursor.valueBuffer("nostock")) {
          MessageBox.warning(AQUtil.translate("scripts", "El proceso asociado al artículo es de modificación. El artículo debe marcarse como \"Sin stock\""), MessageBox.Ok, MessageBox.NoButton);
          return false;
        }
        break;
      }
    ...
    if (!_i.validarTareaProduccion()) {
      return false;
    }
    var refComponenteTareaMala: String = AQUtil.sqlSelect("articuloscomp ac INNER JOIN pr_tipostareapro tt ON ac.idtipotareapro = tt.idtipotareapro", "ac.refcomponente", "ac.refcompuesto = '" + cursor.valueBuffer("referencia") + "' AND tt.idtipoproceso <> '" + idTipoProceso + "'", "articuloscomp,pr_tipostareapro");
    if (refComponenteTareaMala && refComponenteTareaMala != "") {
      MessageBox.warning(AQUtil.translate("scripts", "El componente %1 está asignado a una tarea que no pertenece al proceso %1.").arg(refComponenteTareaMala).arg(idTipoProceso), MessageBox.Ok, MessageBox.NoButton);
      return false;
    }
  }
  return true;
...
function prod_validarTareaProduccion(): Boolean {
  var cursor = this.cursor();
  var _i = this.iface;

  var idComponenteSinTarea: String = AQUtil.sqlSelect("articuloscomp ac LEFT OUTER JOIN articulos a ON ac.refcomponente = a.referencia", "ac.id", "ac.refcompuesto = '" + cursor.valueBuffer("referencia") + "' AND (ac.idtipotareapro = 0 OR ac.idtipotareapro IS NULL) AND (a.tipostock <> 'Grupo base' OR a.referencia IS NULL)", "articuloscomp,articulos");
  if (idComponenteSinTarea && idComponenteSinTarea != "")
  {
    var desComponente: String = AQUtil.sqlSelect("articuloscomp", "desccomponente", "id = " + idComponenteSinTarea);
    MessageBox.warning(AQUtil.translate("scripts", "El componente %1 no tiene tarea de consumo asignada").arg(desComponente), MessageBox.Ok, MessageBox.NoButton);
    return false;
  }
  return true;
}
