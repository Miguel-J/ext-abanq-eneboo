...
function prod_init()
{
  ...
  //  this.child("tdbMoviStock").cursor().setMainFilter("idlineapp = " + cursor.valueBuffer("idlinea"));
  //  this.child("tdbMoviStock").refresh();
  this.child("tdbLotesStock").cursor().setMainFilter("codlote IN (SELECT codlote FROM movistock WHERE idlineapp = " + cursor.valueBuffer("idlinea") + ")");
