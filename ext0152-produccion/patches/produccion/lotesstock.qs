...
function prod_init()
{
  ...
  connect(this.child("tdbRegLoteStock").cursor(), "bufferCommited()", _i, "refrescarTotales");
...
function prod_commonCalculateField(fN: String, cursor: FLSqlCursor): String {
  ...
  switch (fN)
  {
    ...
    case "estado": {
      /// El lote está terminado si no tiene entradas (movimientos positivos) pendientes. Esto cubre el caso de lotes provinientes de líneas de pedido cerradas sin haberse recibido totalmente la mercancía
      var pte = parseFloat(util.sqlSelect("movistock", "SUM(cantidad)", "codlote = '" + codLote + "' AND estado = 'PTE' AND cantidad > 0"));
      if (!pte || isNaN(pte)) {
        pte = 0;
      }
      res = pte > 0 ? "PTE" : "TERMINADO";
      /*
...
function prod_tbnLanzarProceso_clicked()
{
  ...
  */
  ...
  if (!flcolaproc.iface.pub_crearProcesoProd(refProceso, codLote)) {
    MessageBox.warning(util.translate("scripts", "Error al crear el proceso asociado a %1 para el lote %2").arg(refProceso).arg(codLote), MessageBox.Ok, MessageBox.NoButton);
    return false;
  }
