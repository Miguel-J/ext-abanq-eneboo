...
function prod_regularizarStock0(where: String): Boolean {
  ...
  var sFecha: String = fecha.toString();
  var sHora: String = sFecha.right(8);
  ...
  while (curStock.next())
  {
    ...
    curLineasRegStock.setValueBuffer("fecha", sFecha);
    curLineasRegStock.setValueBuffer("hora", sHora);
    ...
    curStock.setValueBuffer("fechaultreg", sFecha);
    curStock.setValueBuffer("horaultreg", sHora);
    curStock.setValueBuffer("cantidad", formRecordregstocks.iface.pub_commonCalculateField("disponible", curStock));
