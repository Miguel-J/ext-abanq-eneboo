...
function interna_validateForm(): Boolean {
  if (!flfactppal.iface.pub_validarCentroCoste(this.cursor().valueBuffer("codcentro")))
  {
    return false;
  }
  return true;
...
function oficial_agregarCentro(): Boolean {
  ...
  if (true)   /// Comprobar configuración y ver si hay que filtrar usuarios
  {
    var idGrupo: String = util.sqlSelect("flusers", "idgroup", "iduser = '" + sys.nameUser() + "'");
    if (idGrupo) {
      if (filtro != "") {
        filtro += " AND ";
      }
