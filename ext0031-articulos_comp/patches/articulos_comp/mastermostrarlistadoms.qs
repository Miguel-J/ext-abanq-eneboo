...
class oficial extends interna
{
  ...
  function dameSelectQry()
  {
    return this.ctx.oficial_dameSelectQry();
  }
...
function oficial_refrescarTabla()
{
  ...
  if (hecho && pte && reservado) {
    ...
    var filtroEstado = "";
    if (hecho) {
      filtroEstado += filtroEstado == "" ? "" : " OR ";
      filtroEstado += "estado = 'HECHO'";
    }
    if (pte) {
      filtroEstado += filtroEstado == "" ? "" : " OR ";
      filtroEstado += "(estado = 'PTE' AND cantidad > 0)";
    }
    if (reservado) {
      filtroEstado += filtroEstado == "" ? "" : " OR ";
      filtroEstado += "(estado = 'PTE' AND cantidad < 0)";
    }
    filtro += filtroEstado != "" ? (" AND (" + filtroEstado + ")") : "";
  }
  //    if (hecho) {
  //      filtroEstado += filtroEstado == "" ? "" : " OR ";
  //      filtroEstado += "estado = 'HECHO'";
  ...
  if (cursor.valueBuffer("fechadesde")) {
    filtro +=  " AND (estado = 'PTE' OR (estado = 'HECHO' AND fechareal >= '" + cursor.valueBuffer("fechadesde") + "'))";
  }
  if (cursor.valueBuffer("fechahasta")) {
    filtro +=  " AND (estado = 'PTE' OR (estado = 'HECHO' AND fechareal <= '" + cursor.valueBuffer("fechahasta") + "'))";
  }
  var qry = new FLSqlQuery;
  with(qry) {
    setTablesList("movistock");
    setSelect(_i.dameSelectQry());
    setFrom("movistock");
...
function oficial_dameSelectQry()
{
  var s = "idmovimiento, referencia, estado, cantidad, fechaprev, fechareal, horareal, codlote, idlineaac, idlineapc, idlineaap, idlineapp, idlineapr, idlineaco, idlineava, idproceso, idlineats";
  if (flfactppal.iface.pub_extension("tpv_multitienda")) {
    s += ",idlineatto, idlineattd";
  }
  return s;
}
...
function oficial_identificarDocumento(qry: FLSqlQuery): Array {
  ...
  if (flfactppal.iface.pub_extension("tpv_multitienda"))
  {
    if (qry.value("idlineatto") && qry.value("idlineatto") != "") {
      datosDoc["tipo"] = "Envío Transferencia";

      with(qryCodigo) {
        setTablesList("tpv_lineasmultitransstock");
        setSelect("idviajemultitrans, codalmadestino");
        setFrom("tpv_lineasmultitransstock");
        setWhere("idlinea = " + qry.value("idlineatto"));
      }
      if (!qryCodigo.exec()) {
        return false;
      }
      if (qryCodigo.first()) {
        datosDoc["codigo"] = qryCodigo.value("idviajemultitrans");
        datosDoc["codigo"] = datosDoc["codigo"] ? datosDoc["codigo"] : "";
        datosDoc["origen"] = sys.translate("Viaje %1 hacia %2").arg(datosDoc["codigo"]).arg(qryCodigo.value("codalmadestino"));
      } else {
        datosDoc["tipo"] = "";
      }
      return datosDoc;
    }
    if (qry.value("idlineattd") && qry.value("idlineattd") != "") {
      datosDoc["tipo"] = "Recepción Transferencia";

      with(qryCodigo) {
        setTablesList("tpv_lineasmultitransstock");
        setSelect("idviajemultitrans, codalmaorigen");
        setFrom("tpv_lineasmultitransstock");
        setWhere("idlinea = " + qry.value("idlineattd"));
      }
      if (!qryCodigo.exec()) {
        return false;
      }
      if (qryCodigo.first()) {
        datosDoc["codigo"] = qryCodigo.value("idviajemultitrans");
        datosDoc["codigo"] = datosDoc["codigo"] ? datosDoc["codigo"] : "";
        datosDoc["origen"] = sys.translate("Viaje %1 desde %2").arg(datosDoc["codigo"]).arg(qryCodigo.value("codalmaorigen"));
      } else {
        datosDoc["tipo"] = "";
      }
      return datosDoc;
...
function oficial_buscarPrimerMov()
{
  ...
  this.child("fdbFechaDesde").setValue("NULL");
  this.child("fdbHoraHasta").setValue("NULL");
  cursor.setNull("fechadesde");
  cursor.setNull("horadesde");
