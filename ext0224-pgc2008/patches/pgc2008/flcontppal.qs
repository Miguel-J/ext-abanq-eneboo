...
class pgc2008 extends oficial
{
  ...
  function copiarEpigrafe(codEpigrafe: String, codEjercicioAnt: String, codEjercicio: String): String {
    return this.ctx.pgc2008_copiarEpigrafe(codEpigrafe, codEjercicioAnt, codEjercicio);
  }
  function copiarGrupoEpigrafe(codGrupoEpigrafe: String, codEjercicioAnt: String, codEjercicio: String): String {
    return this.ctx.pgc2008_copiarGrupoEpigrafe(codGrupoEpigrafe, codEjercicioAnt, codEjercicio);
  }
}
//// PGC 2008 //////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
...
function pgc2008_copiarEpigrafe(codEpigrafe: String, codEjercicioAnt: String, codEjercicio: String): String {
  var util: FLUtil = new FLUtil();
  var curEpigrafeOrigen: FLSqlCursor = new FLSqlCursor("co_epigrafes");
  curEpigrafeOrigen.select("codepigrafe = '" + codEpigrafe + "' AND codejercicio = '" + codEjercicioAnt + "'");
  if (!curEpigrafeOrigen.first())
  {
    return false;
  }
  ...
  var idEpigrafePadre: Number = false;
  var idEpigrafePadreOrigen: Number = curEpigrafeOrigen.valueBuffer("idpadre");
  if (idEpigrafePadreOrigen && idEpigrafePadreOrigen != 0)
  {
    var codEpigrafePadre: String = util.sqlSelect("co_epigrafes", "codepigrafe", "idepigrafe = " + idEpigrafePadreOrigen + " AND codejercicio = '" + codEjercicioAnt + "'");
    if (codEpigrafePadre && codEpigrafePadre != "") {
      idEpigrafePadre = util.sqlSelect("co_epigrafes", "idepigrafe", "codepigrafe = '" + codEpigrafePadre + "' AND codejercicio = '" + codEjercicio + "'");
      if (!idEpigrafePadre) {
        idEpigrafePadre = this.iface.copiarEpigrafe(codEpigrafePadre, codEjercicioAnt, codEjercicio);
        if (!idEpigrafePadre) {
          MessageBox.warning(util.translate("scripts", "Hubo un error al copiar el subgrupo %1 al ejercicio %2.").arg(codEpigrafe).arg(codEjercicio), MessageBox.Ok, MessageBox.NoButton);
          return false;
        }
      }
    }
  ...
  var idGrupoOrigen: Number = curEpigrafeOrigen.valueBuffer("idgrupo");
  var idGrupo: Number = false;
  if (idGrupoOrigen  && idGrupoOrigen != 0)
  {
    var codGrupo: String = util.sqlSelect("co_gruposepigrafes", "codgrupo", "idgrupo = " + idGrupoOrigen + " AND codejercicio = '" + codEjercicioAnt + "'");
    if (codGrupo && codGrupo != "") {
      idGrupo = util.sqlSelect("co_gruposepigrafes", "idgrupo", "codgrupo = '" + codGrupo + "' AND codejercicio = '" + codEjercicio + "'");
      if (!idGrupo) {
        idGrupo = this.iface.copiarGrupoEpigrafe(codGrupo, codEjercicioAnt, codEjercicio);
        if (!idGrupo) {
          MessageBox.warning(util.translate("scripts", "Hubo un error al copiar el grupo del subgrupo %1 al ejercicio %2.").arg(codGrupo).arg(codEjercicio), MessageBox.Ok, MessageBox.NoButton);
          return false;
        }
      }
    }
  ...
  curEpigrafe.setValueBuffer("idpadre", idEpigrafePadre);
  curEpigrafe.setValueBuffer("idgrupo", idGrupo);
  curEpigrafe.setValueBuffer("descripcion", curEpigrafeOrigen.valueBuffer("descripcion"));
  curEpigrafe.setValueBuffer("codejercicio", codEjercicio);
  if (!curEpigrafe.commitBuffer())
  {
    return false;
  }

  var idEpigrafe: String = curEpigrafe.valueBuffer("idepigrafe");
  return idEpigrafe;
}

function pgc2008_copiarGrupoEpigrafe(codGrupo: String, codEjercicioAnt: String, codEjercicio: String): String {
  var util: FLUtil = new FLUtil();
  var curGrupoOrigen: FLSqlCursor = new FLSqlCursor("co_gruposepigrafes");
  curGrupoOrigen.select("codgrupo = '" + codGrupo + "' AND codejercicio = '" + codEjercicioAnt + "'");
  if (!curGrupoOrigen.first())
  {
    return false;
  }
  ...
  var curGrupo: FLSqlCursor = new FLSqlCursor("co_gruposepigrafes");
  curGrupo.setModeAccess(curGrupo.Insert);
  curGrupo.refreshBuffer();
  curGrupo.setValueBuffer("codgrupo", codGrupo);
  curGrupo.setValueBuffer("descripcion", curGrupoOrigen.valueBuffer("descripcion"));
  curGrupo.setValueBuffer("codejercicio", codEjercicio);
  if (!curGrupo.commitBuffer())
  {
    return false;
  }

  var idGrupo: String = curGrupo.valueBuffer("idgrupo");
  return idGrupo;
}
