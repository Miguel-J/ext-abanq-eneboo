...
class oficial extends interna
{
  ...
  function habilitarPorFormato()
  {
    return this.ctx.oficial_habilitarPorFormato();
  }
...
function oficial_habilitarPorFormato()
{
  var _i = this.iface;
  var cursor = this.cursor();
  ...
  switch (cursor.valueBuffer("formato")) {
    case "Abreviado": {
      cursor.setValueBuffer("niveldesglose", "No desglosar");
      sys.disableObj(this, "fdbNivelDesglose");
      break;
    }
    default: {
      try {
        sys.enableObj(this, "fdbNivelDesglose");
      } catch (e) {
        this.child("fdbNivelDesglose").setDisabled(false);
      }
    }
  }
}
