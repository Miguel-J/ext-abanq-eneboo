...
function interna_calculateField(fN)
{
  ...
  switch (_i.ejercicio) {
    default: {
      valor = _i.calculateField2009(fN);
      break;
    }
  }
  return valor;
...
function oficial_calcularCasillas10a18()
{
  var _i = this.iface;
  var cursor = this.cursor();
  ...
  for (var v = 0; v < 7; v = v + 3) {
    ...
    if (porcentajes || porcentajes != "") {
      qryPartidas.setWhere(_i.whereFechas_ + " AND p.casilla303 = '[10]-[18]' AND p.recargo IN (" + porcentajes + ") GROUP BY p.recargo ORDER BY p.recargo ");
      debug(qryPartidas.sql());
      if (!qryPartidas.exec()) {
        return false;
      }
  ...
  var aCasillas = ["[10]-[12]", "[13]-[15]", "[16]-[18]"];
  ...
  /**
    ...
    qryPartidas.setSelect("SUM(p.baseimponible), p.recargo, SUM(p.haber - p.debe)");
    qryPartidas.setFrom("co_asientos a INNER JOIN co_partidas p ON p.idasiento = a.idasiento");
    qryPartidas.setWhere(_i.whereFechas_ + " AND p.casilla303 = '[10]-[18]' AND p.recargo <> 0 GROUP BY p.recargo ORDER BY p.recargo ");
    qryPartidas.setForwardOnly(true);
    if (!qryPartidas.exec()) {
      return false;
    }
    if (qryPartidas.size() > 3) {
      MessageBox.warning(sys.translate("Error al obtener datos de Recargo de equivalencia del Régimen general:\nSe ha encontrado más de tres tipos de recargo"), MessageBox.Ok, MessageBox.NoButton);
      return false;
    }
    var baseImponible = 0;
    var re = 0;
...
function oficial_calcularCasillas19a20()
{
  ...
  qryPartidas.setSelect("SUM(p.baseimponible), SUM(p.haber - p.debe)");
  qryPartidas.setFrom("co_asientos a INNER JOIN co_partidas p ON p.idasiento = a.idasiento");
  qryPartidas.setWhere(_i.whereFechas_ + " AND p.casilla303 = '[19]-[20]'");
  qryPartidas.setForwardOnly(true);
  if (!qryPartidas.exec()) {
    return false;
  }
...
function oficial_calcularCasillas22a23()
{
  var _i = this.iface;
  var cursor = this.cursor();
  ...
  qryPartidas.setFrom("co_asientos a INNER JOIN co_partidas p ON p.idasiento = a.idasiento");
  qryPartidas.setWhere(_i.whereFechas_ + " AND p.casilla303 = '[22]-[23]'");
  qryPartidas.setForwardOnly(true);
  if (!qryPartidas.exec()) {
    return false;
  }
...
function oficial_calcularCasillas24a25()
{
  var _i = this.iface;
  var cursor = this.cursor();
  ...
  qryPartidas.setSelect("SUM(p.baseimponible), SUM(p.debe - p.haber)");
  qryPartidas.setFrom("co_asientos a INNER JOIN co_partidas p ON p.idasiento = a.idasiento");
  qryPartidas.setWhere(_i.whereFechas_ + " AND p.casilla303 = '[24]-[25]'");
  qryPartidas.setForwardOnly(true);
  if (!qryPartidas.exec()) {
    return false;
  }
...
function oficial_calcularCasillas26a27()
{
  var _i = this.iface;
  var cursor = this.cursor();
  ...
  qryPartidas.setSelect("SUM(p.baseimponible), SUM(p.debe - p.haber)");
  qryPartidas.setFrom("co_asientos a INNER JOIN co_partidas p ON p.idasiento = a.idasiento");
  qryPartidas.setWhere(_i.whereFechas_ + " AND p.casilla303 = '[26]-[27]'");
  qryPartidas.setForwardOnly(true);
  if (!qryPartidas.exec()) {
    return false;
  }
...
function oficial_calcularCasillas34()
{
  var _i = this.iface;
  var cursor = this.cursor();
  ...
  qryPartidas.setSelect("SUM(p.baseimponible), SUM(p.debe - p.haber)");
  qryPartidas.setFrom("co_asientos a INNER JOIN co_partidas p ON p.idasiento = a.idasiento");
  qryPartidas.setWhere(_i.whereFechas_ + " AND p.casilla303 = '[34]'");
  qryPartidas.setForwardOnly(true);
  if (!qryPartidas.exec()) {
    return false;
  }
...
function oficial_calcularCasillas43()
{
  ...
  qryPartidas.setSelect("SUM(p.baseimponible), SUM(p.debe - p.haber)");
  qryPartidas.setFrom("co_asientos a INNER JOIN co_partidas p ON p.idasiento = a.idasiento");
  qryPartidas.setWhere(_i.whereFechas_ + " AND p.casilla303 = '[43]'");
  qryPartidas.setForwardOnly(true);
  if (!qryPartidas.exec()) {
    return false;
  }
...
function oficial_calcularCasillas44()
{
  ...
  qryPartidas.setSelect("SUM(p.baseimponible), SUM(p.debe - p.haber)");
  qryPartidas.setFrom("co_asientos a INNER JOIN co_partidas p ON p.idasiento = a.idasiento");
  qryPartidas.setWhere(_i.whereFechas_ + " AND p.casilla303 = '[44]'");
  qryPartidas.setForwardOnly(true);
  if (!qryPartidas.exec()) {
    return false;
  }
