...
class oficial extends interna
{
  ...
  function registroBeneficiario50(curRecibo: FLSqlCursor): String {
    return this.ctx.oficial_registroBeneficiario50(curRecibo);
  }
...
function interna_acceptedForm()
{
  ...
  while (curRecibos.next()) {
    ...
    if (this.iface.zonaBBeneficiario == "58") {
      datos = this.iface.registroBeneficiario50(curRecibos);
      if (!datos || datos.length != this.iface.tamRegistro) {
        debug("Longitud: " + datos.length + " de " + this.iface.tamRegistro);
        debug("Cadena ***" + datos + "***");
        return;
  ...
  var util: FLUtil = new FLUtil();
  MessageBox.information(util.translate("scripts", "Generado fichero de Cuaderno 34A en :\n\n" + this.child("ledFichero").text + "\n\n"), MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
}
...
/** \D Crea el texto del 50º registro de la cabecera del beneficiario
Obligatorio únicamente para pagarés.
@param curRecibo: Cursor del registro de un recibo
@return Texto con los dato para ser volcados a fichero
\end */
function oficial_registroBeneficiario50(curRecibo: FLSqlCursor): String {
  var util: FLUtil = new FLUtil();

  var date: Date = curRecibo.valueBuffer("fechav");
  var fechaV = flfactppal.iface.pub_cerosIzquierda(date.getDate().toString(), 2) + flfactppal.iface.pub_cerosIzquierda(date.getMonth().toString(), 2) + date.getYear().toString().right(2);

  var reg: String = this.iface.zonaABeneficiario + this.iface.zonaBBeneficiario + this.iface.zonaCBeneficiario + this.iface.zonaDBeneficiario;
  // Zona E: Número de dato (3 dígitos) = 910
  reg += "910";
  // Zona F: (36 dígitos) Fecha de vencimieno del pagaré (DDMMAAAA), ajustado a la izquierda completado con blancos.
  reg += flfactppal.iface.pub_espaciosDerecha(fechaV, 36);
  // Zona G: Libre (7 dígitos)
  reg += flfactppal.iface.pub_espaciosDerecha("", 7);
