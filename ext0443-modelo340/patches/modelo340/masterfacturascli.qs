...
class modelo340 extends modelos
{
  ...
  function clave340D(cursor)
  {
    return this.ctx.modelo340_clave340D(cursor);
  }
  ...
  switch (fN) {
    ...
    case "desglose340": {
      if (cursor.valueBuffer("claveoperacion340") == "C") {
        valor = AQUtil.sqlSelect("lineasivafactcli", "COUNT(*)", "idfactura = " + cursor.valueBuffer("idfactura"));
      } else {
        valor = 1;
      }
      break;
...
{
  debug("modelo340_calculaClave340");
  var _i = this.iface;
  var valor = "";
  valor = _i.clave340D(cursor);
  if (valor != "") {
    return valor;
  }
  ...
  debug("modelo340_calculaClave340 vacía");
  return "";
}

function modelo340_clave340D(cursor)
{
  var _i = this.iface;
  var valor = "";
  if (!cursor.isNull("idfacturarect") && cursor.valueBuffer("deabono")) {
    valor = "D";
  }
  return valor;
}
...
function modelo340_clave340C(cursor) /// Si la factura tiene varios IVA
{
  var _i = this.iface;
  var valor = "";
  var numIVA = AQUtil.sqlSelect("lineasivafactcli", "COUNT(*)", "idfactura = " + cursor.valueBuffer("idfactura"));
  if (numIVA > 1) {
    valor = "C";
  }
  return valor;
...
function modelo340_clave340PorArticulo(cursor)
{
  ...
  q.setFrom("lineasfacturascli lf INNER JOIN articulos a ON lf.referencia = a.referencia");
