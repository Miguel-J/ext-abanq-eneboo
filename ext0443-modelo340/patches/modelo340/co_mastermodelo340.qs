...
class oficial extends interna
{
  ...
  function presTelematica2012()
  {
    return this.ctx.oficial_presTelematica2012();
  }
  ...
  function toolButtonAeat_clicked()
  {
    return this.ctx.oficial_toolButtonAeat_clicked();
  }
}
//// OFICIAL /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
function interna_init()
{
  ...
  connect(this.child("toolButtonAeat"), "clicked()", _i, "toolButtonAeat_clicked()");
}
//// INTERNA /////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////
//// OFICIAL /////////////////////////////////////////////////////
function oficial_toolButtonAeat_clicked()
{
  var _i = this.iface;
  var util = new FLUtil();
  ...
  var fechaEjercicio = util.sqlSelect("ejercicios", "fechainicio", "codejercicio = '" + cursor.valueBuffer("codejercicio") + "'");
  var anoEjercicio = fechaEjercicio.getYear();
  debug("anoEjercicio " + anoEjercicio);
  if (parseInt(anoEjercicio) >= 2012) {
    _i.presTelematica2012();
  } else {
    _i.presTelematica();
  }
}
...
\end */
function oficial_presTelematica2012()
{
  var util = new FLUtil();
  var cursor = this.cursor();
  if (!cursor.isValid()) {
  ...
  */
  ...
  while (qryFacturasEmitidas.next()) {
    ...
    nombreDato = util.translate("scripts", "Situación del inmueble");
    if ((contenidoEmitidas.length + 1) !=  385) {
      return this.iface.errorAcumuladoControl(385, nombreDato);
    }
    temp = qryFacturasEmitidas.value("codsituinmueble");
    temp = flfactppal.iface.pub_cerosIzquierda(temp, 1);
    contenidoEmitidas += temp;
    contenidoCSV += "|" + temp;

    nombreDato = util.translate("scripts", "Referencia catastral");
    if ((contenidoEmitidas.length + 1) !=  386) {
      return this.iface.errorAcumuladoControl(386, nombreDato);
    }
    temp = qryFacturasEmitidas.value("refcatastral");
    temp = flfactppal.iface.pub_espaciosDerecha(temp, 25);
    contenidoEmitidas += temp;
    contenidoCSV += "|" + temp;
    nombreDato = util.translate("scripts", "Importe recibido en metálico");
    if ((contenidoEmitidas.length + 1) !=  411) {
      return this.iface.errorAcumuladoControl(411, nombreDato);
    }
    temp = qryFacturasEmitidas.value("importemetalico");
    contenidoEmitidas += flcontmode.iface.pub_formatoNumero(temp, 13, 2);
    contenidoCSV += "|" + this.iface.formatoNumeroCSV(temp, "co_facturasemi340", "importemetalico");

    nombreDato = util.translate("scripts", "Ejercicio metálico");
    if ((contenidoEmitidas.length + 1) !=  426) {
      return this.iface.errorAcumuladoControl(426, nombreDato);
    }
    temp = qryFacturasEmitidas.value("ejerciciometalico");
    temp = flfactppal.iface.pub_espaciosDerecha(temp, 4);
    contenidoEmitidas += temp;
    contenidoCSV += "|" + temp;

    nombreDato = util.translate("scripts", "Importe transmisiones inmuebles sujetas a IVA");
    if ((contenidoEmitidas.length + 1) !=  430) {
      return this.iface.errorAcumuladoControl(430, nombreDato);
    }
    temp = qryFacturasEmitidas.value("importetransinmuebles");
    contenidoEmitidas += flcontmode.iface.pub_formatoNumero(temp, 13, 2);
    contenidoCSV += "|" + this.iface.formatoNumeroCSV(temp, "co_facturasemi340", "importetransinmuebles");

    //Blancos del 445 al 500
    nombreDato = util.translate("scripts", "Espacios fin de registro emitidas");
    if ((contenidoEmitidas.length + 1) !=  445) {
      return this.iface.errorAcumuladoControl(445, nombreDato);
    }
