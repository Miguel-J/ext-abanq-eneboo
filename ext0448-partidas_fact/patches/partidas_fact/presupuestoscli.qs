...
{
  this.iface.__init();
  connect(this.child("tdbPartidas").cursor(), "bufferCommited()", this, "iface.calcularTotales()");
  connect(this.child("tdbPartidas").cursor(), "newBuffer()", this, "iface.filtrarLineas");
...
{
  this.iface.guardar();
  this.child("tdbLineasPresupuestosCli").cursor().insertRecord();
}

function partidas_editarLinea()
{
  this.iface.guardar();
  this.child("tdbLineasPresupuestosCli").editRecord();
}

function partidas_borrarLinea()
{
  this.iface.guardar();
  this.child("tdbLineasPresupuestosCli").deleteRecord();
}
...
function partidas_guardar()
{
  ...
  cursor.select("idpresupuesto = " + idPresupuesto);
  cursor.first();
...
function partidas_filtrarLineas()
{
  ...
  var idPartida: String = curPartidas.valueBuffer("idpartida");
  if (!idPartida || idPartida == "" || this.child("chkMostrarTodas").checked) {
    ...
    filtro = "idpartida = " + idPartida;
  }
  this.child("lblFiltro").text = textoFiltro;
  this.child("tdbLineasPresupuestosCli").setFilter(filtro);
