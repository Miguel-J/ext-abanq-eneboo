...
function lotesEnv_calcularCantidad()
{
  ...
  if (this.child("tdbMoviLote").cursor().size() > 0) {
    ...
    this.child("fdbValorMetrico").setValue(util.sqlSelect("movilote", "valormetrico", "docorigen = 'AP' AND idlineaap = " + this.cursor().valueBuffer("idlinea")));
  } else {
    this.child("fdbReferencia").setDisabled(false);
    this.child("fdbCodEnvase").setDisabled(false);
  }
...
function lotesEnv_calculateField(fN: String): String {
  ...
  switch (fN)
  {
    case "canenvases": {
      if (this.iface.porLotes) {
        res = util.sqlSelect("movilote", "SUM(canenvases)", "idlineaap = " + cursor.valueBuffer("idlinea"));
        res = parseFloat(res);
      } else
