...
class oficial extends interna
{
  ...
  function dameEstadosPtePago(tabla)
  {
    return this.ctx.oficial_dameEstadosPtePago(tabla);
  }
...
function oficial_updateUiTableRec()
{
  ...
  var estadosPtePago = this.iface.dameEstadosPtePago(_i.tdbRec_.tableName);

  _i.tdbRec_.setFilter("fecha>='" + _i.deFromRec_.date.toString() + "' and " +
                       "fecha<='" + _i.deToRec_.date.toString() + "' and " +
                       "estado IN (" + estadosPtePago + ")");
  _i.tdbRec_.refresh(false, true);
}

function oficial_dameEstadosPtePago(tabla)
{
  return "'Emitido', 'Devuelto'";
}
