...
function scab_cargarArrayStocks(): Array {
  ...
  while (qryStocks.next())
  {
    ...
    arrayStocks[i]["articulo"]["referencia"] = qryStocks.value("lt.referencia");
    arrayStocks[i]["codalmacen"] = cursor.valueBuffer("codalmaorigen");
  ...
  while (qryStocks.next())
  {
    ...
    arrayStocks[i]["articulo"] = [];
    arrayStocks[i]["articulo"]["referencia"] = qryStocks.value("lt.referencia");
    arrayStocks[i]["codalmacen"] = cursor.valueBuffer("codalmadestino");
    arrayStocks[i]["cantidad"] = qryStocks.value("SUM(lt.cantidad)");
    i++;
  }
  return arrayStocks;
...
function scab_controlStockCabecera(): Boolean {
  ...
  flfactalma.iface.pub_ponStocksAfectados(arrayAfectados);
