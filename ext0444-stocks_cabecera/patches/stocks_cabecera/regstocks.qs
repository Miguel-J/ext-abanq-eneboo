...
class scab extends oficial
{
  ...
  {
    return this.ctx.scab_commonCalculateField(fN, cursor, oParam);
  }
  ...
  function cargarValesTPV(): Boolean {
    return this.ctx.scab_cargarValesTPV();
  }
  ...
  {
    return this.ctx.scab_dameParamStock();
  }
}
//// STOCKS CABECERA ////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
...
function scab_commonCalculateField(fN, cursor, oParam)
{
  debug("scab_commonCalculateField " + oParam);
  var _i = this.iface;
  var util = new FLUtil;
  var valor;
  ...
  switch (fN) {
      ...
      //    }
      //    case "cantidadval": {
      //      var whereStock:String = this.iface.dameWhereStock(cursor, "ld.codalmacen", "d.fechaemision");
    ...
    case "cantidad": {
      if (fechaMax) {
        debug("hay fechaMax " + fechaMax);
        valor = _i.__commonCalculateField(fN, cursor, oParam);
        break;
      }
      debug("No hay fechaMax");
      var cantidadUltReg: Number = parseFloat(cursor.valueBuffer("cantidadultreg"));
      if (isNaN(cantidadUltReg)) {
        cantidadUltReg = 0;
      }
      valor = cantidadUltReg + parseFloat(cursor.valueBuffer("cantidadap")) + parseFloat(cursor.valueBuffer("cantidadfp")) - parseFloat(cursor.valueBuffer("cantidadac")) - parseFloat(cursor.valueBuffer("cantidadfc")) + parseFloat(cursor.valueBuffer("cantidadts")) - parseFloat(cursor.valueBuffer("cantidadtpv")) + parseFloat(cursor.valueBuffer("cantidadval"));
      break;
    }
    default: {
      valor = _i.__commonCalculateField(fN, cursor, oParam);
...
function scab_calcularCantidad()
{
  ...
  if (sys.isLoadedModule("flfact_tpv")) {
    ...
    cursor.setValueBuffer("cantidadval", this.iface.calculateField("cantidadval"));
  } else {
    cursor.setValueBuffer("cantidadtpv", 0);
    cursor.setValueBuffer("cantidadval", 0);
  }
...
function scab_cargarListaMovimientos(): Boolean {
  ...
  if (sys.isLoadedModule("flfact_tpv"))
  {
    ...
    if (!this.iface.cargarValesTPV()) {
      MessageBox.warning(util.translate("scripts", "Error al cargar los movimientos de vales de TPV"), MessageBox.Ok, MessageBox.NoButton);
      return false;
    }
  }
  return true;
...
function scab_crearMovimiento(qryMovimientos: FLSqlQuery, factor: Number, origen: String): Array {
  var util: FLUtil = new FLUtil;
  var campoHora: String;
  var campoCodigo: String;
  switch (origen)
  {
    case "VAL": {
      campoHora = "d.fechaemision";
      campoCodigo = "d.referencia";
      break;
    }
    default: {
      campoHora = "d.fecha";
  ...
  var fecha: Date = qryMovimientos.value(campoHora);
  var hora: Date = qryMovimientos.value("d.hora")
                   debug("origen = " + origen + " Hora = " + hora);
...
function scab_mostrarDocumento(fila: Number, col: Number)
{
  ...
  switch (origen) {
    ...
    case "VAL": {
      curDocumento = new FLSqlCursor("tpv_vales");
      curDocumento.select("referencia = '" + idDocumento + "'");
      break;
    }
    case "TS": {
...
function scab_descripcionOrigen(origen: String): Boolean {
  ...
  switch (origen)
  {
    ...
    case "VAL": {
      descripcion = util.translate("scripts", "Devolución TPV");
      break;
    }
    case "RS": {
...
function scab_dameIdDocumento(origen: String, qryMovimientos: FLSqlQuery): Boolean {
  ...
  switch (origen)
  {
    ...
    case "VAL": {
      idDocumento = qryMovimientos.value("d.referencia");
      break;
    }
    case "RS": {
      break;
...
function scab_cargarVentasTPV(): Boolean {
  ...
  qryMovimientos.setFrom("tpv_comandas d INNER JOIN tpv_lineascomanda ld ON d.idtpv_comanda = ld.idtpv_comanda");
  qryMovimientos.setWhere(this.iface.dameWhereStock(cursor, "d.codalmacen"));
  qryMovimientos.setForwardOnly(true);
  if (!qryMovimientos.exec())
  {
...
function scab_cargarValesTPV(): Boolean {
  var util: FLUtil = new FLUtil;
  var cursor: FLSqlCursor = this.cursor();

  var qryMovimientos: FLSqlQuery = new FLSqlQuery;
  qryMovimientos.setTablesList("tpv_vales,tpv_lineasvale");
  qryMovimientos.setSelect("d.fechaemision, d.hora, ld.cantidad, d.referencia");
  qryMovimientos.setFrom("tpv_vales d INNER JOIN tpv_lineasvale ld ON d.referencia = ld.refvale");
  qryMovimientos.setWhere(this.iface.dameWhereStock(cursor, "ld.codalmacen", "d.fechaemision"));
  qryMovimientos.setForwardOnly(true);
  if (!qryMovimientos.exec())
  {
    return false;
  ...
  while (qryMovimientos.next())
  {
    this.iface.listaMovimientos_[iMovimiento] = this.iface.crearMovimiento(qryMovimientos, 1, "VAL");
    iMovimiento++;
  }
  return true;
}
...
function scab_dameParamStock()
{
  var _i = this.iface;
  var oPS = _i.__dameParamStock();
  oPS.idPedido = undefined;
  return oPS;
}
//// STOCKS CABECERA ////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
