/***************************************************************************
                 seleccionrecibosanticiposprov.qs  -  description
                             -------------------
...
function oficial_incluirRecibo()
{
  ...
  if (aListado && aListado.length > 0) {
    formRecordanticiposprov.iface.pub_ponListaRecibos(aListado);
  } else {
    formRecordanticiposprov.iface.pub_ponListaRecibos(false);
  }
...
function oficial_calcularImporte(listado)
{
  ...
  if (listado && listado.length > 0) {
    total = AQUtil.sqlSelect("recibosprov", "SUM(importe)", "idrecibo IN (" + listado + ")");
  }
  total = isNaN(total) ? 0 : total;
  total = AQUtil.formatoMiles(AQUtil.roundFieldValue(total, "recibosprov", "importe"));
  return total;
}
