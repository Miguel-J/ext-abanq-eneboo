...
function barCode_lineasTallaColor_clicked()
{
  ...
  var curLineas: FLSqlCursor = this.child("tdbLineasAlbaranesProv").cursor();
  ...
  if (!this.iface.lineasTallaColor("lineasalbaranesprov", "idalbaran", cursor.valueBuffer("idalbaran"), referencia, habilitar))
    return;
...
/** \D Lanza el formulario para insertar tallas y colores en pedidos, albaranes y facturas de proveedor
...
function barCode_lineasTallaColor(tabla: String, campoPadre: String, valorCP: String, referencia: String, habilitar: Boolean): Boolean {
  ...
  util.sqlDelete("lineastallacol", "usuario = '" + sys.nameUser() + "'");

  var f: Object = new FLFormSearchDB("lineastallacol");
...
/** \D Actualiza las líneas de facturación de proveedor para un determinado artículo en base a los datos introducidos por el usuario en el formulario de introducción de datos de tallas y colores
...
function barCode_generarLineasTC(curLineasTC: FLSqlCursor): Boolean {
  ...
  {
    if (curLineasTC.valueBuffer("tabla") == "lineasalbaranesprov") {
      ...
      if (idLineaPedido != "0") {
        ...
        flfacturac.iface.restarCantidadProv(idLineaPedido, this.iface.curLinTC.valueBuffer("idlinea"));
        flfacturac.iface.actualizarEstadoPedidoProv(idPedido);
      }
    }
  ...
  /*
    if (!util.sqlDelete(curLineasTC.valueBuffer("tabla"), curLineasTC.valueBuffer("campopadre") + " = " + curLineasTC.valueBuffer("valorcampopadre") + " AND referencia = '" + referencia + "'"))
      return false;
  */
...
function barCode_datosLineasTC(curLineasTC: FLSqlCursor, datosTC: Array): Boolean {
  ...
  this.iface.curLinTC.setValueBuffer("pvpsindto", formRecordlineaspedidosprov.iface.pub_commonCalculateField("pvpsindto", this.iface.curLinTC));
  this.iface.curLinTC.setValueBuffer("pvptotal", formRecordlineaspedidosprov.iface.pub_commonCalculateField("pvptotal", this.iface.curLinTC));
  if (curLineasTC.valueBuffer("tabla") == "lineasalbaranesprov")
  {
