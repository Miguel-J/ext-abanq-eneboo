...
function barCode_lineasTallaColor(tabla: String, campoPadre: String, valorCP: String, referencia: String, habilitar: Boolean): Boolean {
  ...
  try {
    if (*/
  this.iface.generarLineasTC(curLineasTC)/*) {
    curLineasTC.commit();
  } else {
...
function barCode_datosLineasTC(curLineasTC: FLSqlCursor, datosTC: Array): Boolean {
  ...
  if (flfactppal.iface.pub_extension("numeros_linea"))
  {
    this.iface.curLinTC.setValueBuffer("numlinea", formRecordlineaspedidoscli.iface.pub_commonCalculateField("numlinea", this.iface.curLinTC));
  }
  return true;
}
//// TALLAS Y COLORES POR BARCODE ///////////////////////////////
