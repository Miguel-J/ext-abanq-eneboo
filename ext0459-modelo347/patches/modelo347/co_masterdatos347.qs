...
function oficial_lanzar()
{
  ...
  if (!idModelo) {
    MessageBox.warning(util.translate("scripts", "Debe asociar el informe a un modelo ya existente"), MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton, "AbanQ");
    return;
  }

  var tipo = cursor.valueBuffer("tipo");
  var CC = (tipo == "Clientes") ? "B" : "A";
  var where = "f.idmodelo = " + idModelo + " AND f.clavecodigo = '" + CC + "'";
  var cifNif = cursor.valueBuffer("cifnif");
  if (cifNif && cifNif != "") {
    where += "AND f.nifdeclarado = '" + cifNif + "'";
  }
  var consulta = CC == "B" ? "co_datos347" : "co_datos347prov";
  var report = cursor.valueBuffer("listado") ? "co_datos347list" : "co_datos347";
  var orden: String = cursor.valueBuffer("orden");
  var orderBy = (orden == "Nombre" ? "f.nifdeclarado" : "f.apellidosnombrers");
  flcontmode.iface.pub_lanzar(cursor, consulta, where + " ORDER BY " + orderBy, report);
...
function oficial_textoFecha(fecha: String): String {
  ...
  {
    case "01": {
      textoMes = util.translate("scripts", "Enero");
      break;
    }
    case "02": {
      textoMes = util.translate("scripts", "Febrero");
      break;
    }
    case "03": {
      textoMes = util.translate("scripts", "Marzo");
      break;
    }
    case "04": {
      textoMes = util.translate("scripts", "Abril");
      break;
    }
    case "05": {
      textoMes = util.translate("scripts", "Mayo");
      break;
    }
    case "06": {
      textoMes = util.translate("scripts", "Junio");
      break;
    }
    case "07": {
      textoMes = util.translate("scripts", "Julio");
      break;
    }
    case "08": {
      textoMes = util.translate("scripts", "Agosto");
      break;
    }
    case "09": {
      textoMes = util.translate("scripts", "Septiembre");
      break;
    }
    case "10": {
      textoMes = util.translate("scripts", "Octubre");
      break;
    }
    case "11": {
      textoMes = util.translate("scripts", "Noviembre");
      break;
    }
    case "12": {
      textoMes = util.translate("scripts", "Diciembre");
      break;
    }
  }
...
function oficial_mostrarValores(nodo: FLDomNode, campo: String): String {
  var util = new FLUtil;
  var cursor: FLSqlCursor = this.cursor();
  var cantidad = util.sqlSelect("co_modelo347", "importeminimo", "idmodelo = " + cursor.valueBuffer("idmodelo"));
