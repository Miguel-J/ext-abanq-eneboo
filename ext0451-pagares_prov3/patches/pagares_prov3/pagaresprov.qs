...
class oficial extends interna
{
  ...
  function datosRecibo(curRecibo: FLSqlCursor, curPagare: FLSqlCursor): Boolean {
    return this.ctx.oficial_datosRecibo(curRecibo, curPagare);
  }
  ...
  function construirFiltro(cursor: FLSqlCursor): String {
...
function oficial_agregarRecibo(): Boolean {
  ...
  */
  ...
  var filtro: String = this.iface.construirFiltro(cursor);
  ...
  for (var i: Number = 0; i < recibos.length; i++)
  {
    ...
    try {
      ...
      curT.rollback();
      MessageBox.critical(util.translate("scripts", "Hubo un error en la asociación del recibo al pagaré:") + "\n" + e, MessageBox.Ok, MessageBox.NoButton);
    }
  }
...
function oficial_asociarReciboPagare(idRecibo: String, curPagare: FLSqlCursor): Boolean {
  ...
  if (curRecibos.next())
  {
    ...
    curRecibos.setModeAccess(curRecibos.Edit);
    curRecibos.refreshBuffer();
    if (!this.iface.datosRecibo(curRecibos, curPagare))
      return false;
    idFactura = curRecibos.valueBuffer("idfactura");
  ...
  this.iface.curPagosDev.setValueBuffer("nogenerarasiento", curPagare.valueBuffer("nogenerarasiento"));
...
function oficial_datosRecibo(curRecibo: FLSqlCursor, curPagare: FLSqlCursor): Boolean {
  curRecibo.setValueBuffer("estado", "Pagaré");
  curRecibo.setValueBuffer("fechav", curPagare.valueBuffer("fechav"));

  return true;
}

function oficial_datosPagosDev(idRecibo: String, curPagare: FLSqlCursor): Boolean {
  this.iface.curPagosDev.setValueBuffer("numero", curPagare.valueBuffer("numero"));
  return true;
}
...
function oficial_construirFiltro(cursor: FLSqlCursor): String {
  var filtro: String = "codproveedor = '" + cursor.valueBuffer("codproveedor") + "' AND estado IN ('Emitido', 'Devuelto') AND fecha <= '" + cursor.valueBuffer("fecha") + "'";
