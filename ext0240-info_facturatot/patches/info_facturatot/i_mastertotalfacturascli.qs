/***************************************************************************
                 i_mastertotalfacturascli.qs  -  description
                             -------------------
    begin                : lun nov 1 2007
...
function oficial_lanzar()
{
  ...
  var where: String = "i_totalfacturas_buffer.idsesion = '" + sys.idSession() + "'"; // AND i_totalfacturascli.id = " + seleccion;
...
function oficial_rellenarDatos(cursor: FLSqlCursor)
{
  ...
  var editable: String = cursor.valueBuffer("editable");
  if (editable == "Sí")
    editable = 1;
  if (editable == "No")
    editable = 0;
  ...
  if (codagente)
    where += " AND codagente = '" + codagente + "'";
  if (codserie)
    where += " AND codserie = '" + codserie + "'";
  if (editable)
    where += " AND editable = '" + editable + "'";

  where += " GROUP BY fecha";

  q = new FLSqlQuery();
  q.setTablesList("facturascli");
  q.setSelect("fecha,SUM(neto) as neto,SUM(totaliva) as totaliva,SUM(totalrecargo) as totalrecargo,SUM(totalirpf) as totalirpf,SUM(totaleuros) as totaleuros,COUNT(*) as numero");
  ...
  while (q.next()) {
    ...
    curTab.setValueBuffer("totalrecargo", curTab.valueBuffer("totalrecargo") + q.value(3));
    curTab.setValueBuffer("totalirpf",   curTab.valueBuffer("totalirpf") + q.value(4));
    curTab.setValueBuffer("totaleuros",  curTab.valueBuffer("totaleuros") + q.value(5));
    if (curTab.valueBuffer("numero") == "")
      curTab.setValueBuffer("numero", 0);
    curTab.setValueBuffer("numero",      parseInt(curTab.valueBuffer("numero")) + q.value(6));
