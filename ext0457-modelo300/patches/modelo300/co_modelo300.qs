...
class interna
{
  ...
  function calculateField2006(fN: String): String {
    return this.ctx.interna_calculateField2006(fN);
  }
  function calculateField2007(fN: String): String {
    return this.ctx.interna_calculateField2007(fN);
  }
}
//// INTERNA /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
class oficial extends interna
{
  ...
  function bufferChanged2006(fN)
  {
    return this.ctx.oficial_bufferChanged2006(fN);
  }
  function bufferChanged2007(fN)
  {
    return this.ctx.oficial_bufferChanged2007(fN);
  }
}
//// OFICIAL /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
function interna_calculateField(fN): String {
  ...
  {
    case "2006": {
      valor = this.iface.calculateField2006(fN);
      break;
    }
    default: {
      valor = this.iface.calculateField2007(fN);
      break;
    }
  }
  return valor;
}
function interna_calculateField2006(fN): String {
...
function interna_calculateField2007(fN): String {
  var util: FLUtil = new FLUtil;
  var cursor: FLSqlCursor = this.cursor();
  var valor;
  switch (fN)
  {
    ...
    case "cuotadedtotal":
      ...
      parseFloat(this.child("fdbCuotaDedAI").value()) +
      parseFloat(this.child("fdbCuotaComRE").value()) +
      parseFloat(this.child("fdbCuotaRegIn").value()) + parseFloat(this.child("fdbCuotaRegApli").value());
      break;
...
function interna_validateForm(): Boolean {
  ...
  var cursor: FLSqlCursor = this.cursor();
  var resultado: Number = parseFloat(cursor.valueBuffer("resliquid"));
  if (!resultado || isNaN(resultado))
  {
    resultado = 0;
  }
  if (cursor.valueBuffer("periodo") != "4T" && cursor.valueBuffer("idtipodec") == "D")
  {
...
function oficial_bufferChanged(fN)
{
  ...
  switch (this.iface.ejercicio) {
    case "2006": {
      this.iface.bufferChanged2006(fN);
      break;
    }
    default: {
      this.iface.bufferChanged2007(fN);
      break;
    }
  }
}

function oficial_bufferChanged2006(fN)
{
  var cursor: FLSqlCursor = this.cursor();
  switch (fN) {
    case "cuotarg1":
...
function oficial_bufferChanged2007(fN)
{
  var cursor: FLSqlCursor = this.cursor();
  switch (fN) {
    case "cuotarg1":
...
function oficial_calcularValores()
{
  ...
  for (var i: Number = 0; i < ivas.length; i++) {
    ...
    while (qryRegIVA.next()) {
      baseImponible += parseFloat(qryRegIVA.value(0));
      cuota += parseFloat(qryRegIVA.value(1));
    }
  ...
  while (qryRegIVA.next()) {
    sumaDebe += parseFloat(qryRegIVA.value(2));
