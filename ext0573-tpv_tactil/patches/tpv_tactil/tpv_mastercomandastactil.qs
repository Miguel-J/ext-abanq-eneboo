...
class oficial extends interna
{
  ...
  var pbnEstablecerNombre: Object;
  ...
  function cambiarNombreComanda_clicked()
  {
    return this.ctx.oficial_cambiarNombreComanda_clicked();
  }
  function cambiarNombreComanda(idComanda: Number)
  {
    return this.ctx.oficial_cambiarNombreComanda(idComanda);
  }
}
//// OFICIAL /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
class ifaceCtx extends head
{
  ...
  function pub_cambiarNombreComanda(idComanda: Number)
  {
    return this.cambiarNombreComanda_clicked(idComanda);
  }
}
...
function interna_init()
{
  ...
  this.iface.pbnEstablecerNombre = this.child("pbnEstablecerNombre");
  ...
  connect(this.iface.pbnEstablecerNombre, "clicked()", this, "iface.cambiarNombreComanda_clicked()");

  var orden: Array = ["nombrecomanda", "fecha", "hora", "estado", "total", "pendiente", "codtpv_agente", "codtpv_puntoventa"];
...
function oficial_imprimirQuick_clicked()
{
  /*
    ...
    var util:FLUtil = new FLUtil();
    var pv:String = flfact_tpv.iface.pub_valorDefectoTPV("codterminal");
...
function oficial_imprimirQuick(codComanda: String, impresora: String)
{
  ...
  var codPuntoVenta: String = flfact_tpv.iface.pub_valorDefectoTPV("codterminal");
...
function oficial_filtroVentas(): String {
  var filtro: String = "";
  var util: FLUtil = new FLUtil;
  debug("solo hoy " + this.iface.ckbSoloHoy.on);
  if (this.iface.ckbSoloHoy.on)
  {
    debug("entra");
  ...
  debug("filtro " + filtro);
  if (this.iface.ckbSoloPV.on)
  {
    var codTerminal: String = flfact_tpv.iface.pub_valorDefectoTPV("codterminal");
    if (codTerminal) {
      if (filtro != "")
        filtro += " AND ";
...
function oficial_cambiarNombreComanda_clicked()
{
  var util: FLUtil = new FLUtil;
  var cursor: FLSqlCursor = this.cursor();
  ...
  this.iface.cambiarNombreComanda(idComanda);
}

function oficial_cambiarNombreComanda(idComanda: Number)
{
  var util: FLUtil;
  ...
  var texto: String = util.sqlSelect("tpv_comandas", "nombrecomanda", "idtpv_comanda = " + idComanda);
  var idUsuario: String = sys.nameUser();
  var f: Object = new FLFormSearchDB("tpv_tecladoletras");
  var curTeclado: FLSqlCursor = f.cursor();

  curTeclado.select("idusuario = '" + idUsuario + "'");
  if (!curTeclado.first())
    curTeclado.setModeAccess(curTeclado.Insert);
  else
    curTeclado.setModeAccess(curTeclado.Edit);

  f.setMainWidget();
  curTeclado.refreshBuffer();
  curTeclado.setValueBuffer("idusuario", idUsuario);
  curTeclado.setValueBuffer("texto", texto);

  var nombre: String = f.exec("texto");
  if (!nombre)
    return false;

  util.sqlUpdate("tpv_comandas", "nombrecomanda", nombre, "idtpv_comanda = " + idComanda);
}
//// OFICIAL /////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
