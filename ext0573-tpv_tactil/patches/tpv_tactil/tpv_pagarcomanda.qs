...
function interna_init()
{
  ...
  this.iface.total = parseFloat(util.roundFieldValue(this.iface.total, "tpv_cantidadpago", "importe"));
...
function oficial_calcularCambio()
{
  ...
  debug("this.iface.total " + this.iface.total);
  debug("importe " + this.cursor().valueBuffer("importe"));
  debug("cambio " + cambio);
...
function oficial_modificarCantidad(numero: Number)
{
  ...
  if (this.child("fdbImporte").editor().hasSelectedText) {
    cursor.setValueBuffer("importe", "");
  }
