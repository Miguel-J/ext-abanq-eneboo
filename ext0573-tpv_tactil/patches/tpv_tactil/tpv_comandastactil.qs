...
class oficial extends interna
{
  ...
  function probarInsercionLinea()
  {
    return this.ctx.oficial_probarInsercionLinea();
  }
  function dameIndiceCelda(fil: Number, col: Number)
  {
  ...
  function datosLineaVenta(cantidad)
  {
    return this.ctx.oficial_datosLineaVenta(cantidad);
  }
  ...
  function pideDtoUsuario()
  {
    return this.ctx.oficial_pideDtoUsuario();
  }
  function aplicarDtoLineas(porDto: Number)
  {
  ...
  function aplicarDtoLinea(idLinea: Number, porDto: Number)
  {
  ...
  function realizarPago(codPago: String)
  {
    return this.ctx.oficial_realizarPago(codPago);
  }
  function crearPago(importe: Number, codPago: String)
  {
  ...
  function dameObjetoArticulo()
  {
    return this.ctx.oficial_dameObjetoArticulo();
  }
  function datosIdArticuloLineaVenta(oArticulo)
  {
    return this.ctx.oficial_datosIdArticuloLineaVenta(oArticulo);
  }
  function ordenarLineasComanda()
  {
    return this.ctx.oficial_ordenarLineasComanda();
  }
  function establecerGeometria()
  {
    return this.ctx.oficial_establecerGeometria();
  }
...
function interna_init()
{
  ...
  this.iface.establecerGeometria();
  ...
  this.iface.ordenarLineasComanda();
  ...
  switch (cursor.modeAccess()) {
    ...
    case cursor.Insert: {
      ...
      if (codTerminal && util.sqlSelect("tpv_puntosventa", "codtpv_puntoventa", "codtpv_puntoventa ='" + codTerminal + "'")) {
        ...
        var agente = util.sqlSelect("tpv_puntosventa", "codtpv_agente", "codtpv_puntoventa ='" + codTerminal + "'");
        if (!agente || agente == "") {
...
function interna_calculateField(fN: String)
{
  ...
  switch (fN) {
    ...
    */
    case "total": {
      var neto = parseFloat(this.iface.calculateField("neto"));
      var totalIva = parseFloat(this.iface.calculateField("totaliva"));
      valor = neto + totalIva;
      break;
    }
    /** \C
...
function oficial_construirArrayArticulos(nivel: String)
{
  ...
  if (i != 0) {
    ...
    var aux = this.iface.paginas[nivel].toString().split(".");
...
function oficial_mostrarPagina()
{
  ...
  this.iface.anchoArticulos = sizeTabla.width - 5; /// Evita que la pantalla crezca en cada selección
  this.iface.altoArticulos = sizeTabla.height - 5;

  /// Necesario para extensión tallas y colores
  this.iface.tableArticulos.setNumRows(this.iface.numRows);
  this.iface.tableArticulos.setNumCols(this.iface.numCols);

  var indice = 0;
  var alto = this.iface.altoArticulos / this.iface.numRows;
  var ancho = this.iface.anchoArticulos / this.iface.numCols;
...
function oficial_dameIndiceCelda(fil: Number, col: Number)
{
  ...
  var indice = (this.iface.numRows * this.iface.numCols * this.iface.paginaActual[this.iface.nivelActual]) + (fil * this.iface.numCols) + col;
  if (this.iface.arrayArticulos[this.iface.nivelActual].length <= indice) {
    return -1;
  }
  return indice;
...
function oficial_clickedTabla(fil: Number, col: Number)
{
  ...
  var indice = this.iface.dameIndiceCelda(fil, col);
  if (indice < 0) {
    return false;
  }

  var tipo = this.iface.arrayArticulos[this.iface.nivelActual][indice]["tipo"];
...
\end */
function oficial_probarInsercionLinea()
{
  ...
  var referencia = this.iface.arrayArticulos[this.iface.nivelActual][indice]["id"];
  if (!referencia || referencia == "") {
    return false;
  }
  var oArticulo = this.iface.dameObjetoArticulo();
  oArticulo.referencia = referencia;
  this.iface.insertarLinea(oArticulo, cantidad);
...
/** \D Inserta una línea para un determinado artículo y cantidad
  @param  oArticulo: Objeto con los identificadores del artículos
  @param  Cantidad
\end */
function oficial_insertarLinea(oArticulo, cantidad)
{
  var util = new FLUtil;
  var cursor = this.cursor();
  debug("oficial_insertarLinea");
  this.iface.curLineas = this.child("tdbLineasComanda").cursor();
  if (cursor.modeAccess() == cursor.Insert) {
    debug("oficial_insertarLinea 1");
  ...
  this.iface.curLineas.setValueBuffer("idtpv_comanda", idComanda);
  if (!this.iface.datosIdArticuloLineaVenta(oArticulo)) {
    return false;
  }
  if (!this.iface.datosLineaVenta(cantidad)) {
    return false;
  }
...
function oficial_datosIdArticuloLineaVenta(oArticulo)
{
  var util = new FLUtil;
  ...
  if (referencia != "") {
    ...
    this.iface.curLineas.setValueBuffer("descripcion", util.translate("scripts", "VARIOS"));
  }
  return true;
}
...
\end */
function oficial_datosLineaVenta(cantidad)
{
  var util = new FLUtil;
  var cursor = this.cursor();

  var referencia = this.iface.curLineas.valueBuffer("referencia");
  ...
  if (this.iface.precioLibre && this.iface.precioLibre != 0) {
    ...
    pvp = formRecordtpv_lineascomanda.iface.pub_commonCalculateField("pvpunitario", this.iface.curLineas);;
  }
  var codImpuesto: String;
  if (this.iface.impuestoLibre && this.iface.impuestoLibre != "") {
    codImpuesto = this.iface.impuestoLibre;
  } else {
    codImpuesto = formRecordtpv_lineascomanda.iface.pub_commonCalculateField("codimpuesto", this.iface.curLineas);
  }
  ...
  this.iface.curLineas.setValueBuffer("costeunitario", formRecordtpv_lineascomanda.iface.pub_commonCalculateField("costeunitario", this.iface.curLineas));
...
function oficial_nivelAnterior()
{
  ...
  var nivelAnt = util.sqlSelect("nivelestpv", "codnivelsup", "codnivel = '" + this.iface.nivelActual + "'");
...
function oficial_mostrarTotal()
{
  ...
  //  var util:FLUtil;
  //
  //  var idComanda= this.cursor().valueBuffer("idtpv_comanda");
  //  if(!idComanda)
  //    return false;
  //
  //  var total= parseFloat(util.sqlSelect("tpv_lineascomanda","SUM(pvptotal)","idtpv_comanda = " + idComanda));
  //  if(!total)
  //    total = 0;
  //
...
{
  var porDto = this.iface.pideDtoUsuario();
...
function oficial_pideDtoUsuario()
{
  var util: FLUtil;
  var cursor = this.cursor();
  var idUsuario = sys.nameUser();

  var f = new FLFormSearchDB("tpv_dtocomandatactil");
  var curDescuento = f.cursor();
  ...
  var porDto = f.exec("pordto");
  return porDto;
}

function oficial_aplicarDtoLineas(porDto: Number)
{
  var util: FLUtil;
  var cursor = this.cursor();
  if (isNaN(porDto) && !porDto) {
  ...
  var curTrans = new FLSqlCursor("empresa");
...
function oficial_borrarLinea()
{
  ...
  if (this.iface.tbnSeleccionarTodo.on) {
    ...
    var curLineas = this.child("tdbLineasComanda").cursor();
    var idLineaComanda = curLineas.valueBuffer("idtpv_linea");
    if (!idLineaComanda) {
      return false;
    }
...
function oficial_imprimir()
{
  ...
  var util = new FLUtil();
  var pv = flfact_tpv.iface.pub_valorDefectoTPV("codterminal");
  if (!pv) {
...
function oficial_sumarUno(idLinea: Number)
{
  if (!idLinea)
    return false;
...
function oficial_aplicarDtoLinea(idLinea: Number, porDto: Number)
{
  if (!idLinea) {
    return false;
  }
...
function oficial_restarUno(idLinea: Number)
{
  if (!idLinea)
    return false;
...
{
  var util: FLUtil;
  var pagoEfectivo = util.sqlSelect("tpv_datosgenerales", "pagoefectivo", "1=1");
...
{
  var util: FLUtil;
  var pagoTarjeta = util.sqlSelect("tpv_datosgenerales", "pagotarjeta", "1=1");
...
function oficial_realizarPago(codPago: String)
{
  var util = new FLUtil();
  var cursor = this.cursor();
  ...
  var idUsuario = sys.nameUser();
  var f = new FLFormSearchDB("tpv_pagarcomanda");
  ...
  var impEntregado = parseFloat(entregado);
  var cambio = 0;
  if (impEntregado == 0)
    return false;
...
*/
function oficial_crearPago(importe: Number, codPago: String)
{
  var util = new FLUtil;
  var cursor = this.cursor();
  var fecha = new Date;
...
function oficial_precioLibre_clicked()
{
  ...
  if (this.iface.tbnPrecioLibre.on) {
    ...
    this.iface.tbnTarifaEspecial.on = false;
    var idUsuario = sys.nameUser();
    var f = new FLFormSearchDB("tpv_pagolibre");
    var curPagoLibre = f.cursor();
...
function oficial_productoGenerico_clicked()
{
  ...
  var oArticulo = this.iface.dameObjetoArticulo();
  this.iface.insertarLinea(oArticulo, 1);
  this.iface.precioLibre = 0;
  this.iface.impuestoLibre = "";
}

function oficial_dameObjetoArticulo()
{
  var oArticulo = new Object();
  oArticulo.referencia = "";
...
function oficial_comandas_clicked()
{
  ...
  var idUsuario = sys.nameUser();
  var f = new FLFormSearchDB("tpv_comandastactil");
  f.setMainWidget();

  var idComanda = f.exec("idtpv_comanda");
  if (!idComanda)
    return false;
...
function oficial_tbnStock_clicked()
{
  ...
  switch (tipo) {
    case "articulo": {
      oArticulo.referencia = this.iface.arrayArticulos[this.iface.nivelActual][indice]["id"];
      break;
    }
  }
  if (oArticulo.referencia && oArticulo.referencia != "") {
...
function oficial_ordenarLineasComanda()
{
  var orden = ["cantidad", "descripcion", "pvptotal", "codimpuesto"];
  this.child("tdbLineasComanda").setOrderCols(orden);
}

function oficial_establecerGeometria()
{
  var qG = new FLSqlQuery;
  qG.setTablesList("tpv_datosgenerales");
  qG.setSelect("geofija, xtactil, ytactil, wtactil, htactil");
  qG.setFrom("tpv_datosgenerales");
  qG.setWhere("1 = 1");
  if (!qG.exec()) {
    return;
  }
  ...
  var g = new Rect(qG.value("xtactil"), qG.value("ytactil"), qG.value("wtactil"), qG.value("htactil"));
  formRecordtpv_comandastactil.obj().setGeometry(g);
}
