/***************************************************************************
                 rappelarticulos.qs  -  description
                             -------------------
...
function rappel_validateForm(): Boolean {
  ...
  */
  ...
  if (cursor.modeAccess() == cursor.Insert || cursor.modeAccess() == cursor.Edit)
  {
    ...
    query.setTablesList("rappelarticulos");
    query.setSelect("limiteinferior");
    query.setFrom("rappelarticulos");
    query.setWhere("((limiteinferior BETWEEN " + limiteInferior + " AND " + limiteSuperior + ") OR (" + "limitesuperior BETWEEN " + limiteInferior + " AND " + limiteSuperior + "))" + " AND referencia = '" + cursor.valueBuffer("referencia") + "' AND id <> " + cursor.valueBuffer("id") + ";");
    if (!query.exec())
      return false;
    if (query.next()) {
