...
function oficial_incluirFila(fila: Number, col: Number)
{
  ...
  try {
    this.iface.tblAgentes_.repaintContents();
  } catch (e) {}
...
function oficial_actualizar()
{
  ...
  var filtro: String = flfactppal.iface.pub_obtenFiltroFacturas(false, cursor.valueBuffer("fechadesde"), cursor.valueBuffer("fechahasta"), cursor.valueBuffer("codejercicio"), this.iface.docLiquidacion_);
  var qryDocs: FLSqlQuery = new FLSqlQuery;
  qryDocs.setTablesList(tabla);
  qryDocs.setSelect(tabla + ".codagente");
  qryDocs.setFrom(tabla);
  qryDocs.setWhere(filtro + " GROUP BY " + tabla + ".codagente");
  ...
  while (qryDocs.next()) {
    codAgente = qryDocs.value(tabla + ".codagente");
...
function oficial_insertarLineaTabla(codAgente)
{
  var util: FLUtil = new FLUtil;
  var cursor: FLSqlCursor = this.cursor();

  var liq;
  var filtro: String = flfactppal.iface.pub_obtenFiltroFacturas(codAgente, cursor.valueBuffer("fechadesde"), cursor.valueBuffer("fechahasta"), cursor.valueBuffer("codejercicio"), this.iface.docLiquidacion_);
  ...
  liq = flfactppal.iface.pub_calcularLiquidacionAgente(filtro, this.iface.docLiquidacion_);
  //  liq = AQUtil.formatoMiles(AQUtil.roundFieldValue(liq, "facturasprov", "total"));
  liq = AQUtil.roundFieldValue(liq, "facturasprov", "total");
  this.iface.tblAgentes_.setText(numLinea, this.iface.COL_TOTAL, liq);
...
function oficial_tbnFacturas_clicked()
{
  ...
  var curDocs: FLSqlCursor = f.cursor();

  var filtro: String = flfactppal.iface.pub_obtenFiltroFacturas(codAgente, cursor.valueBuffer("fechadesde"), cursor.valueBuffer("fechahasta"), cursor.valueBuffer("codejercicio"), this.iface.docLiquidacion_);

  curDocs.setMainFilter(filtro);

  f.setMainWidget();
  var idDoc: String = f.exec(clave);
}
//// OFICIAL /////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
