...
function oficial_pbnCalcularValores()
{
  ...
  var porRetencion: Number = 0;
  var totalBase: Number = 0;
  ...
  while (qryFacturasProv.next()) {
    ...
    base = qryFacturasProv.value("SUM(f.neto)");
    if (isNaN(base)) {
      base = 0;
    }
    retencion = qryFacturasProv.value("SUM(f.totalirpf)");
    if (isNaN(retencion)) {
      retencion = 0;
    }
    porRetencion = base != 0 ? retencion * 100 / base : 0;
    porRetencion = util.roundFieldValue(porRetencion, "co_detalle180", "porretencion");

    curDetalle.setValueBuffer("base", base);
    curDetalle.setValueBuffer("porretencion", porRetencion);
    curDetalle.setValueBuffer("retencion", retencion);
    if (!curDetalle.commitBuffer()) {
      return false;
    }
    totalBase += parseFloat(base);
    totalRetencion += parseFloat(retencion);
  }
  this.child("fdbNumPercep").setValue(qryFacturasProv.size());
  this.child("fdbBaseRetenciones").setValue(totalBase);
  this.child("fdbRetenciones").setValue(totalRetencion);
