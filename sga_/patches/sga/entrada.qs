...
function interna_init()
{
  ...
  var codBarras: String = util.sqlSelect("articulos", "codbarras", "referencia = '" + cursor.valueBuffer("referencia") + "'");
  if (!codBarras)
    codBarras = "";
  this.child("ledArticulo").setText(codBarras);

  var canEnvase: String = util.sqlSelect("envases", "cantidad", "referencia = '" + cursor.valueBuffer("referencia") + "'");
  if (!canEnvase) {
    canEnvase = 1;
  }
  this.child("fdbCanEnvase").setValue(canEnvase);
