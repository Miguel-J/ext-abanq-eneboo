/***************************************************************************
                 venfacturasprov.qs  -  description
                             -------------------
...
function interna_validateForm(): Boolean {
  ...
  \end */
  var sumaImportes: Number = parseFloat(util.sqlSelect("venfacturasprov", "SUM(importe)", "idfactura = " + cursor.valueBuffer("idfactura") + " AND id <> " + cursor.valueBuffer("id")));
  if (!sumaImportes || isNaN(sumaImportes))
    sumaImportes = 0;
  var nuevoAplazado: Number = importe + sumaImportes;
  var total: Number = cursor.cursorRelation().valueBuffer("total");
  total = parseFloat(util.roundFieldValue(total, "facturasprov", "total"));
  nuevoAplazado = parseFloat(util.roundFieldValue(nuevoAplazado, "facturasprov", "total"));

  if (nuevoAplazado > total)
  {
    MessageBox.critical(util.translate("scripts", "La suma de los importes no puede superar el importe total de la factura"), MessageBox.Ok, MessageBox.NoButton);
    return false;
  }
  return true;
}
...
function interna_calculateField(fN: String): String {
  ...
  switch (fN)
  {
    case "importe": {
      ...
      valor = util.roundFieldValue(valor, "venfacturasprov", "importe");
      break;
    }
    case "aplazado": {
