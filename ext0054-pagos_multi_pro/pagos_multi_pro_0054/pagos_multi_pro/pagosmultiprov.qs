...
{
  function pubPagosMulti(context)
  {
    ifaceCtx(context);
  }
}
...
function interna_calculateField(fN: String): String {
  ...
  switch (fN)
  {
    ...
    case "idsubcuenta":
      ...
      break;
      /** \C
      La cuenta bancaria por defecto será la asociada al proveedor (Cuenta de pago).
      \end */
    case "codcuenta":
      res = false;
...
function oficial_excluirDePagoMulti(idPagoMulti: String, idRecibo: String): Boolean {
  ...
  {
    var codRecibo: String = util.sqlSelect("recibosprov", "codigo", "idrecibo = " + idRecibo);
    MessageBox.warning(util.translate("scripts", "No puede eliminarse el pago del recibo %1 correspondiente al pago múltiple número %2\nEl pago a eliminar no es el último pago del recibo").arg(codRecibo).arg(idPagoMulti), MessageBox.Ok, MessageBox.NoButton);
    return false;
  }
...
/** \C Si hay uno o más recibos asignados al pago múltiple, ciertos campos deben deshabilitarse, como el código de proveedor, la divisa, o el código de cuenta.
\end */
