...
class pedProvCli extends oficial
{
  ...
  function tbnPedidosCli()
  {
    return this.ctx.pedProvCli_tbnPedidosCli();
  }
...
{
  function pubPedProvCli(context)
  {
    ifaceCtx(context);
  }
...
{
  this.iface.tbnPedidosCli();
  this.iface.tdbRecords.refresh();
  this.iface.procesarEstado();
}

function pedProvCli_tbnPedidosCli()
{
  ...
  if (!this.iface.pedidosSel_ || this.iface.pedidosSel_.length == 0) {
    return;
  }
...
function pedProvCli_datosPedidoProvCli(indice: Number): Boolean {
  ...
  with(this.iface.curPedidoProvCli_)
  {
    ...
    setValueBuffer("fechaentrada", hoy);
...
function pedProvCli_datosLineaPedidoProvCli(curLineasCli: FLSqlCursor, curLineasProv: FLSqlCursor, idPedido: Number): String {
  ...
  with(curLineasProv)
  {
    ...
    if (flfactppal.iface.pub_extension("tallcol_barcode")) {
      setValueBuffer("barcode", curLineasCli.valueBuffer("barcode"));
      setValueBuffer("talla", curLineasCli.valueBuffer("talla"));
      setValueBuffer("color", curLineasCli.valueBuffer("color"));
    }
