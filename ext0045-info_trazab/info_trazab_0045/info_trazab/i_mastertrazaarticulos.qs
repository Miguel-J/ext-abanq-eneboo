...
function oficial_rellenarDatos(cursor: FLSqlCursor)
{
  ...
  if (codGrupo)
    where += " AND c.codgrupo = '" + codGrupo + "'";

  var wE: String = this.iface.whereExtraFVenta();
  where += wE;

  q = new FLSqlQuery();
  q.setTablesList("facturascli,lineasfacturascli,albaranescli,clientes");
  q.setSelect("l.referencia,l.descripcion,f.nombrecliente,l.cantidad,l.pvptotal,a.fecha,a.codigo,f.fecha,f.codigo,f.idfacturarect,l.dtopor,l.pvpunitario,c.codcliente,c.codgrupo");
  q.setFrom("facturascli f " +
            ...
            "LEFT JOIN clientes c ON f.codcliente = c.codcliente");
  q.setWhere(where);
  debug(q.sql());
