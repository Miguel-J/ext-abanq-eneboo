/***************************************************************************
                 pr_paramcortadora.qs  -  description
                             -------------------
...
function oficial_cargarDatos()
{
  ...
  this.child("fdbMaxGrosorCm").setValue(eParam.attribute("MaxGrosorCm"));
}
...
function oficial_guardarDatos(): FLDomDocument {
  ...
  var maxGrosorCm: String = cursor.valueBuffer("maxgrosorcm");
  if (isNaN(maxGrosorCm))
  {
    maxGrosorCm = 0;
  }
  maxGrosorCm = util.roundFieldValue(maxGrosorCm, "pr_paramcortadora", "maxgrosorcm");
