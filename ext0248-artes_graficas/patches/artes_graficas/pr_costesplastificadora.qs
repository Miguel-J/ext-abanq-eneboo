/***************************************************************************
         pr_costesplastificadora.qs  -  description
                             -------------------
...
function interna_validateForm(): Boolean {
  ...
  {
    if (util.sqlSelect("pr_costesplastificadora", "id", where + " AND copiasmin IS NULL")) {
    ...
    if (util.sqlSelect("pr_costesplastificadora", "id", where + " AND (copiasmin <= " + cursor.valueBuffer("copiasmin") + " OR copiasmin IS NULL) AND (copiasmax >= " + cursor.valueBuffer("copiasmin") + " OR copiasmax IS NULL)"))
    {
      MessageBox.warning(util.translate("scripts", "El límite inferior de copias indicado se solapa con un intervalo ya existente."), MessageBox.Ok, MessageBox.NoButton);
      return false;
    }
  ...
  {
    if (util.sqlSelect("pr_costesplastificadora", "id", where + " AND copiasmax IS NULL")) {
    ...
    if (util.sqlSelect("pr_costesplastificadora", "id", where + " AND (copiasmin <= " + cursor.valueBuffer("copiasmax") + " OR copiasmin IS NULL) AND (copiasmax >= " + cursor.valueBuffer("copiasmax") + " OR copiasmax IS NULL)"))
    {
      MessageBox.warning(util.translate("scripts", "El límite superior de copias indicado se solapa con un intervalo ya existente."), MessageBox.Ok, MessageBox.NoButton);
      return false;
    }
